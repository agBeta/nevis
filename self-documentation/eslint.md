...
## Environment
There are many ways to specify `env`. These environments are not mutually exclusive, so you can define more than one at a time. We specified `env` inside our eslint configuration file.  
Additionally, according to [eslint docs](https://eslint.org/docs/latest/use/configure/language-options#specifying-environments), you can specify environments with a comment inside of a JavaScript file, use the following format:
```js
