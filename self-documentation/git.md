...
Another resource: How I use Git Worktrees in my developer workflow By Niki Nisi.
```bash
# Create a new branch nicknisi/new-feature based on 
# currect HEAD of origin/main and put the contents 
# inside new-feature folder.
git worktree add -b nicknisi/new-feature new-feature origin/main
```
...
#### best practice
Based on [this SO](https://stackoverflow.com/a/6626178) you should **not** use `.gitignore` files within the repository to ignore files that only appear for you, such as editor-specific swap files. You should be placing those rules in a global .gitignore file so that they apply to all repositories, or in `.git/info/excludes`.
